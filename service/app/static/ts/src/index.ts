...
const userImg = document.getElementsByClassName(
  ' user-img'
...
function changeBtnColor(): void {
  if (
    userId.value.length >= 1 &&
    userPasswd.value.length >= 6 &&
    !submitBtn.classList.contains('onButton')
  ) {
    submitBtn.classList.add('onButton');
  }
  if (
    (userId.value.length < 1 || userPasswd.value.length < 6) &&
    submitBtn.classList.contains('onButton')
  ) {
...
function showPasswd(e: Event): void {
  ...
  node.addEventListener('click', (): void => {
    ...
    userPasswd.type == 'password'
      ? hide(userPasswd, node)
      : show(userPasswd, node);
  });
 ...
 */
setInterval(() => {
  let another: number = setLoadingDot(count);
  count = another;
}, 500);
// 올라가는 진행바
